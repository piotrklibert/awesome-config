...
__haxe_Template.prototype.resolve = function(self,v) 
  ...
  while (_g_head ~= nil) do 
    ...
    if (value == nil) then 
      ...
      tmp1 = (function() 
        local _hx_2
        if ((_G.type(o1) == "string") and ((String.prototype[v] ~= nil) or (v == "length"))) then 
        _hx_2 = true; elseif (o1.__fields__ ~= nil) then 
        _hx_2 = o1.__fields__[v] ~= nil; else 
        _hx_2 = o1[v] ~= nil; end
        return _hx_2
      end )();
    else
...
__haxe_Template.prototype.parseTokens = function(self,data) 
  ...
  while (__haxe_Template.splitter:match(data)) do repeat 
    ...
    if (__lua_lib_luautf8_Utf8.byte(data, p.pos + 1) == 58) then 
      local pos1 = p.pos + 2;
      local len1 = p.len - 4;
      if ((len1 == nil) or (len1 > (pos1 + __lua_lib_luautf8_Utf8.len(data)))) then 
      ...
      tokens:add(_hx_o({__fields__={p=true,s=true,l=true},p=__lua_lib_luautf8_Utf8.sub(data, pos1 + 1, pos1 + len1),s=false,l=nil}));
    ...
    if ((len2 == nil) or (len2 > (pos2 + __lua_lib_luautf8_Utf8.len(data)))) then 
    ...
    if (pos2 < 0) then 
      pos2 = __lua_lib_luautf8_Utf8.len(data) + pos2;
    end;
    if (pos2 < 0) then 
      pos2 = 0;
    end;
    data = __lua_lib_luautf8_Utf8.sub(data, pos2 + 1, pos2 + len2);until true
...
__haxe_Template.prototype.parse = function(self,tokens) 
  ...
  local kwdEnd = function(kwd) 
    ...
    if (__lua_lib_luautf8_Utf8.sub(p, pos1 + 1, pos1 + len) == kwd) then 
      ...
      local pos2 = length;
      local len1 = nil;
      if ((len1 == nil) or (len1 > (pos2 + __lua_lib_luautf8_Utf8.len(p)))) then 
        len1 = __lua_lib_luautf8_Utf8.len(p);
      else
        if (len1 < 0) then 
          len1 = __lua_lib_luautf8_Utf8.len(p) + len1;
        end;
      end;
      if (pos2 < 0) then 
        pos2 = __lua_lib_luautf8_Utf8.len(p) + pos2;
      end;
      if (pos2 < 0) then 
        pos2 = 0;
      end;
      local _g_offset = 0;
      local _g_s = __lua_lib_luautf8_Utf8.sub(p, pos2 + 1, pos2 + len1);
      while (_g_offset < __lua_lib_luautf8_Utf8.len(_g_s)) do 
        _g_offset = _g_offset + 1;
        local c = __lua_lib_luautf8_Utf8.byte(_g_s, (_g_offset - 1) + 1);
        if (c == 32) then 
          pos = pos + 1;
        else
          break;
  ...
  local pos3 = kwdEnd("if");
  if (pos3 > 0) then 
    local pos4 = pos3;
    local len2 = __lua_lib_luautf8_Utf8.len(p) - pos3;
    if ((len2 == nil) or (len2 > (pos4 + __lua_lib_luautf8_Utf8.len(p)))) then 
    ...
    if (pos4 < 0) then 
      pos4 = __lua_lib_luautf8_Utf8.len(p) + pos4;
    end;
    if (pos4 < 0) then 
      pos4 = 0;
    end;
    p = __lua_lib_luautf8_Utf8.sub(p, pos4 + 1, pos4 + len2);
    ...
    if (t1.p == "end") then 
      ...
      if (t1.p == "else") then 
        ...
        if ((len3 == nil) or (len3 > (pos5 + __lua_lib_luautf8_Utf8.len(_this)))) then 
        ...
        if (pos5 < 0) then 
          pos5 = __lua_lib_luautf8_Utf8.len(_this) + pos5;
        end;
        if (pos5 < 0) then 
          pos5 = 0;
        end;
        t1.p = __lua_lib_luautf8_Utf8.sub(_this, pos5 + 1, pos5 + len3);
        eelse = self:parse(tokens);
      end;
    end;
    do return __haxe__Template_TemplateExpr.OpIf(e, eif, eelse) end;
  end;
  local pos6 = kwdEnd("foreach");
  if (pos6 >= 0) then 
    local pos7 = pos6;
    local len4 = __lua_lib_luautf8_Utf8.len(p) - pos6;
    if ((len4 == nil) or (len4 > (pos7 + __lua_lib_luautf8_Utf8.len(p)))) then 
    ...
    if (pos7 < 0) then 
      pos7 = __lua_lib_luautf8_Utf8.len(p) + pos7;
    end;
    if (pos7 < 0) then 
      pos7 = 0;
    end;
    p = __lua_lib_luautf8_Utf8.sub(p, pos7 + 1, pos7 + len4);
...
__haxe_Template.prototype.parseExpr = function(self,data) 
  ...
  while (__haxe_Template.expr_splitter:match(data)) do 
    ...
    local r = __lua_lib_luautf8_Utf8.find(p1, "\"", startIndex, true);
  ...
  if (__lua_lib_luautf8_Utf8.len(data) ~= 0) then 
    ...
    while (_g_offset < __lua_lib_luautf8_Utf8.len(_g_s)) do 
      ...
      if (c ~= 32) then 
        ...
        l:add(_hx_o({__fields__={p=true,s=true},p=__lua_lib_luautf8_Utf8.sub(data, pos1 + 1, pos1 + len1),s=true}));
        break;
      end;
    end;
  end;
...
__haxe_Template.prototype.run = function(self,e) 
  ...
  elseif (tmp) == 3 then 
    ...
    local _this5 = _this4;
    _this5.length = _this5.length + __lua_lib_luautf8_Utf8.len(str3);
  elseif (tmp) == 4 then 
    local l = e[2];
    local _g_head = l.h;
    while (_g_head ~= nil) do 
  ...
  elseif (tmp) == 5 then 
    ...
    elseif not _hx_status then 
      ...
      elseif not _hx_status then 
        ...
        local _hx_2 = _hx_result
        local e6 = _hx_2
        _G.error(Std.string("Cannot iter on ") .. Std.string(Std.string(v2)),0);
      elseif _hx_result ~= _hx_pcall_default then
        return _hx_result
      end;
    elseif _hx_result ~= _hx_pcall_default then
  ...
  elseif (tmp) == 6 then 
    local params = e[3];
    local m = e[2];
    local v4 = Reflect.field(self.macros, m);
    local pl = Array.new();
    local old = self.buf;
    pl:push(_hx_bind(self,self.resolve));
    local _g_head1 = params.h;
    ...
    local _hx_status, _hx_result = pcall(function() 
        ...
        local str4 = Std.string(Reflect.callMethod(self.macros,v4,pl));
        _G.table.insert(_this6.b, str4);
        local _this7 = _this6;
        _this7.length = _this7.length + __lua_lib_luautf8_Utf8.len(str4);
      return _hx_pcall_default
    end)
    if not _hx_status and _hx_result == "_hx_pcall_break" then
    elseif not _hx_status then 
      ...
      local msg = Std.string(Std.string(Std.string(Std.string(Std.string(Std.string("Macro call ") .. Std.string(m)) .. Std.string("(")) .. Std.string(plstr)) .. Std.string(") failed (")) .. Std.string(Std.string(e7))) .. Std.string(")");
      _G.error(msg,0);
    elseif _hx_result ~= _hx_pcall_default then
      return _hx_result
    end; end;
...
MyLog.new = {}
MyLog.__name__ = true
MyLog.log = function(obj) 
  local file = __sys_io_File.append(MyLog.path);
  file:writeString(MyLog.template:execute(_hx_o({__fields__={obj=true},obj=__ex_Inspect.inspect(obj)})));
  file:close();
end
...
Main.__name__ = true
Main.boing = function() 
  local uris = _hx_tab_array({[0]="foo/bar/x/y/z", "foo/baz/bar", "foo/bar"}, 3);
  local i = 0;
  local _g = 0;
  while (_g < uris.length) do 
    local u = uris[_g];
    _g = _g + 1;
    local idx = 1;
    local ret = _hx_tab_array({}, 0);
    ...
    while (idx ~= nil) do 
      local newidx = 0;
      if (__lua_lib_luautf8_Utf8.len("/") > 0) then 
        newidx = __lua_lib_luautf8_Utf8.find(u, "/", idx, true);
      else
        if (idx >= __lua_lib_luautf8_Utf8.len(u)) then 
          newidx = nil;
        else
          newidx = idx + 1;
        end;
      ...
      if (newidx ~= nil) then 
        local match = __lua_lib_luautf8_Utf8.sub(u, idx, newidx - 1);
        ret:push(match);
        idx = newidx + __lua_lib_luautf8_Utf8.len("/");
      else
        ret:push(__lua_lib_luautf8_Utf8.sub(u, idx, __lua_lib_luautf8_Utf8.len(u)));
        idx = nil;
      end;
    end;
    local parts = ret;
    local clb = __tink_core__Callback_Callback_Impl_.fromNiladic(function() 
      local _hx_status, _hx_result = pcall(function() 
      
          local tmp = _hx_o({__fields__={s=true},s="df"});
          MyLog.log("");
          local bool = Reflect.isObject(tmp);
          MyLog.log(_hx_o({__fields__={o=true},o=bool}));
        return _hx_pcall_default
      end)
      if not _hx_status and _hx_result == "_hx_pcall_break" then
      elseif not _hx_status then 
    ...
    i = i + 1;
    __ex_Timer.new(_hx_o({__fields__={timeout=true,callback=true,autostart=true,single_shot=true},timeout=i,callback=clb,autostart=true,single_shot=true}));
  end;
end
Main.main = function() 
  Main.boing();
...
__haxe_ds_List.prototype.toString = function(self) 
  ...
  while (l ~= nil) do 
    if (first) then 
      ...
      s_length = s_length + __lua_lib_luautf8_Utf8.len(str1);
    end;
    local str2 = Std.string(l.item);
    _G.table.insert(s_b, str2);
    s_length = s_length + __lua_lib_luautf8_Utf8.len(str2);
    l = l.next;
  end;
  local str3 = "}";
  _G.table.insert(s_b, str3);
  s_length = s_length + __lua_lib_luautf8_Utf8.len(str3);
...
local _hx_static_init = function()
  ...
  __haxe_Template.hxKeepArrayIterator = (function() 
    ...
    _hx_1 = _hx_o({__fields__={hasNext=true,next=true},hasNext=function(self) 
      do return cur_length < _gthis.length end;
    end,next=function(self) 
      cur_length = cur_length + 1;
      do return _gthis[cur_length - 1] end;
  ...
  MyLog.path = "/home/cji/portless/lua/awesome-config/haxeshigh/finished";
  
  MyLog.template = __haxe_Template.new("-- ::obj::\n---\n");
