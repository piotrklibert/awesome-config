...
local __externs_Tag = _G.require("awful.tag")
local __externs_Wibox = _G.require("wibox")
local __externs_awful_widget_Taglist = _G.require("awful.widget.taglist")
local __externs_wibox_WidgetTools = _G.require("wibox.widget")
...
local __lib_InspectFilters = _hx_e()
local __lib_NativeInspect = _G.require("inspect")
...
__lib_InspectFilters.new = {}
__lib_InspectFilters.__name__ = true
__lib_InspectFilters.normal = function(value,path) 
  if (path == nil) then 
    do return value end;
  end;
  local last = path[#path];
  if ((last ~= __lib_NativeInspect.METATABLE) and (last ~= "__fields__")) then 
    do return value end;
  else
    do return nil end;
  end;
...
__log_Log.log = function(x,opts,infos) 
  ...
  while (true) do repeat 
    ...
    if (_hx_tmp.length == 1) then 
      ...
      local opts = ({depth = 2});
      local o = ({process = __lib_InspectFilters.normal});
      o.process = __lib_InspectFilters.normal;
      if (opts ~= nil) then 
        local _g = 0;
        local _g1 = Reflect.fields(opts);
        while (_g < _g1.length) do 
          local x = _g1[_g];
          _g = _g + 1;
          o[x] = Reflect.field(opts, x);
        end;
      end;
      x = __lib_NativeInspect.inspect(x, o);
      break;
    end;
...
__taglist_Switcher.tag_left = function() 
  local value = Std.parseInt(__externs_Screen.focused().selected_tag.name);
  if (value == nil) then 
    _G.error(__safety_NullPointerException.new("Null pointer in .sure() call"),0);
  end;
...
__taglist_TaglistRow.prototype.makeTaglist = function(self,s) 
  local button = awful.button(_hx_e(), 1, function(x) 
    local sel = __externs_Screen.focused().selected_tag;
    __externs_Tag.viewtoggle(x);
    __externs_Tag.viewtoggle(sel);
  end);
  local conf = ({screen = s, filter = self:makeFilterFun(), buttons = ({button})});
  do return __utils_lua__LuaTools_LuaTable_Impl_.fromObject(__externs_awful_widget_Taglist(conf)) end
end
...
__taglist_Taglist.mkWidget = function(s) 
  ...
  local ret = ({id = "grid", spacing = 6, layout = __externs_Wibox.layout.fixed.vertical});
  local _g = 0;
  ...
  do return __externs_wibox_WidgetTools(ret) end;
end
...
__taglist_Taglist.prototype.enable = function(self) 
  if (self.tagListBox == __haxe_ds_Option.None) then 
    self.tagListBox = __haxe_ds_Option.Some(__externs_Wibox(__taglist_Taglist.wiboxConfig));
  end;
  ...
  elseif (tmp1) == 1 then 
    _G.error(__haxe_Exception.thrown("None in OptionTools.sure() call"),0); end;
  local value = __externs_Screen.focused();
  if (value == nil) then 
    _G.error(__safety_NullPointerException.new("Null pointer in .sure() call"),0);
  end;
...
__taglist_Taglist.prototype.setup = function(self,wibox,widget) 
  ...
  local _tmp_0 = ({widget});
  _tmp_0.margins = 15;
  _tmp_0.layout = _hx_funcToField(__externs_Wibox.container.margin);
  ...
  _tmp_1.widget = _hx_funcToField(__externs_Wibox.container.background);
